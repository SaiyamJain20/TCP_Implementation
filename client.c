...
int main(int argc, char *argv[])
{
    ...
    while(sendto(sock, "connect", strlen("connect"), 0, (struct sockaddr *)&servaddr, serverAddrlen) < 0);
    ...
    while (1)
    {
        ...
        {
            sendto(sock, "exit", strlen("exit"), 0, (struct sockaddr *)&servaddr, serverAddrlen);
            printf("Quitting\n");
            fflush(stdout);
            break;
        }
        ...
        {
            sendto(sock, "continue", strlen("continue"), 0, (struct sockaddr *)&servaddr, serverAddrlen);
        }
        ...
        for (int i = 0; i < total_chunks; i++)
        {
            ...
            sendto(sock, &chunks[i], sizeof(struct Chunk), 0, (struct sockaddr *)&servaddr, serverAddrlen);
            ...
            int ack_num;
            int n = recvfrom(sock, &ack_num, sizeof(int), MSG_DONTWAIT, (struct sockaddr *)&servaddr, &serverAddrlen);
            if (n > 0)
            {
            ...
            {
                gettimeofday(&now, NULL);
                sendto(sock, &chunks[queue[start]], sizeof(struct Chunk), 0, (struct sockaddr *)&servaddr, serverAddrlen);
        ...
        while (recvfrom(sock, req, sizeof(req), 0, (struct sockaddr *)&servaddr, &serverAddrlen) < 0)
            ;
        ...
        {
            struct Chunk chunk;
            while (recvfrom(sock, &chunk, sizeof(struct Chunk), 0, (struct sockaddr *)&servaddr, &serverAddrlen) < 0)
                ;
            j++;
            if (j % DONT_SEND_ACK == 0)
                continue;
            ...
            sendto(sock, &ack_num, sizeof(int), 0, (struct sockaddr *)&servaddr, serverAddrlen);
